...
app.use((req, res, next) => {
  const allowed = ['https://score.getcreativemoney.com'];
...
const apiLimiter = rateLimit({
  ...
  legacyHeaders: false,
  message: { success: false, reason: 'too_many_requests' }
});
app.use('/api/', apiLimiter);

// ── Body size limit + JSON parsing ───────────────────────────────
app.use(express.json({ limit: '10kb' }));

// ── Gzip compression for all responses (reduces payload ~70%) ──
app.use(compression());


// ── Static files with Expires / Cache-Control headers ──
app.use(express.static(path.join(__dirname, 'public'), {
  etag:         true,
  lastModified: true,
  setHeaders: (res, filePath) => {
    if (filePath.endsWith('.html')) {
      // HTML: revalidate every time so users always get fresh content
      res.setHeader('Cache-Control', 'no-cache');
    } else if (/\.(jpg|jpeg|png|gif|ico|webp|svg)$/i.test(filePath)) {
      ...
      res.setHeader('Expires', new Date(Date.now() + 604800 * 1000).toUTCString());
    } else {
...
const BREVO_BAND_TEMPLATE = { f: 2, g: 5, m: 6, l: 7 };
...
app.post('/api/submit', async (req, res) => {
  const { name, email, phone, answers, entityType, sector } = req.body;

  if (!email || !email.includes('@')) {
    return res.status(400).json({ success: false, error: 'Invalid email' });
  }

  // Block re-submission if email is already on the general list
  const alreadyScored = await checkEmailOctopus(EO_LIST_GENERAL, email);
  if (alreadyScored) {
    return res.json({ success: false, reason: 'already_scored' });
  }
  ...
  try {
    const [eoRes, fbRes, brevoRes] = await Promise.allSettled([
      addToEmailOctopus({ name: name || '', email, phone: phone || '', bandKey: band.key, entityType: entityType || '', sector: sector || '' }),
      saveToFirebase({ name: name || '', email, phone: phone || '', score, band, pillars, entityType: entityType || '', sector: sector || '' }),
      sendWelcomeEmail({ name: name || '', email, score, bandName: band.band, bandKey: band.key })
    ]);
...
// 📬  EmailOctopus
// ================================================================
function addToEmailOctopus({ name, email, phone, bandKey, entityType, sector }) {
  function postToList(listId) {
    return new Promise((resolve, reject) => {
      const body = JSON.stringify({
        api_key:       EO_API_KEY,
        email_address: email,
        fields:        { FirstName: name, Phone: phone, EntityType: entityType, Sector: sector },
        status:        'SUBSCRIBED'
      });
      const options = {
...
// 🔥  Firebase
// ================================================================
async function saveToFirebase({ name, email, phone, score, band, pillars, entityType, sector }) {
...
// ================================================================
// ✉️  Brevo API — send welcome email based on band
// ================================================================
function sendWelcomeEmail({ name, email, score, bandName, bandKey }) {
  return new Promise((resolve, reject) => {
    const templateId = BREVO_BAND_TEMPLATE[bandKey] || 2;
    const body = JSON.stringify({
      templateId,
      to:         [{ email, name }],
